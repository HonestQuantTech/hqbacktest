...
| `DataView.portal` 私有 | 字段名 `_portal`（下划线私有约定），**策略无法**通过 `view.portal.get_bars(sym, future_date)` 读到 `visible_through` 之后的数据；所有数据访问走 `view.history` / `view.current_price` / `view.universe`。 |
